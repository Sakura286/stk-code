...
void saveFrameBuff(const char*, GLuint, GLuint, GLuint);
GLint get_current_fbo();
void print_fbo_info(GLuint, GLuint);
uint8_t half_to_256(uint16_t);

inline void my_own_debug_start() { }
inline void my_own_debug_end() { }
...
void saveFrameBuff(const char* fileName, GLuint new_fbo, GLuint width, GLuint height)
{
    ...
    char ab_FileName[80];
    strcpy(ab_FileName, fileName);
    strcat(ab_FileName, ".ab.ascii");
    FILE* ab_File = fopen(ab_FileName, "w");
    if (ab_File == 0)
        printf("!!!!! ab file cannot open !!!!!\n");
    fprintf(bFile, "P6\n%d %d\n255\n", width, height);
    ...
    switch (type)
    {
        case GL_HALF_FLOAT:
            {
            printf("In GL_HALF_FLOAT, filename is %s\n", fileName);
        ...
        case GL_UNSIGNED_BYTE:
            {
            printf("In GL_UNSIGNED_BYTE, filename is %s\n", fileName);
                                    ...
                                    RTT *rtts)
{
    ...
    // Simulate camera defects from there
    {
        ...
        if (isRace && UserConfigParams::m_bloom && (physics == NULL || !physics->isDebug()))
        {
            ...
            saveFrameBuff("/usb0/stkimg/01-in-fbo.ppm", 21, 1024, 768);
            saveFrameBuff("/usb0/stkimg/02-bloom1024-before-blit.ppm", 31, 1024, 1024);
            FrameBuffer::blit(*in_fbo, rtts->getFBO(FBO_BLOOM_1024),
                              GL_COLOR_BUFFER_BIT, GL_LINEAR);
            saveFrameBuff("/usb0/stkimg/03-bloom1024-after-blit.ppm", 31, 1024, 1024);

            rtts->getFBO(FBO_BLOOM_512).bind();
            // glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE);
            // glClearColor(0.0, 1.1, 0.0, 1.1);
            // glClear(GL_COLOR_BUFFER_BIT);
            glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_FALSE);

            saveFrameBuff("/usb0/stkimg/04-bloom512-before-render.ppm", 32, 512, 512);
            renderBloom(rtts->getRenderTarget(RTT_BLOOM_1024));
            saveFrameBuff("/usb0/stkimg/05-bloom512-after-render.ppm", 32, 512, 512);
            ...
            // Blur
            saveFrameBuff("/usb0/stkimg/06-tmp512-before-gaussian.ppm", 33, 512, 512);
            renderGaussian6Blur(rtts->getFBO(FBO_BLOOM_512),
                                rtts->getFBO(FBO_TMP_512), 1., 1.);
            saveFrameBuff("/usb0/stkimg/07-tmp512-after-gaussian.ppm", 33, 512, 512);
            saveFrameBuff("/usb0/stkimg/08-bloom512-after-gaussian.ppm", 32, 512, 512);
            ...
            saveFrameBuff("/usb0/stkimg/09-in-fbo-after-bloom.ppm", 21, 1024, 768);
            ...
            glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE);
            my_own_debug_end();
            //saveFrameBuff("/usb0/stkimg/bloom_19-end.ppm", 1024, 768);
        } // end if bloom
        PROFILER_POP_CPU_MARKER();
    }
